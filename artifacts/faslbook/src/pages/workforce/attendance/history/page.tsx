...
interface AttRecord {
  ...
  date: string; status: "present" | "halfDay" | "absent";
}

type Filter = "week" | "month" | "custom";

const STATUS_LABELS: Record<string, { label: string; bg: string; color: string }> = {
  present:  { label: "Present",  bg: "#E8F5E9", color: GREEN },
...
export default function WorkforceAttendanceHistoryPage() {
    ...
    (async () => {
      ...
      try {
        const [attSnap, empSnap] = await Promise.all([
          ...
          getDocs(query(collection(db, "employees"), where("organizationId", "==", orgId))),
        ]);
        const empList = empSnap.docs.map((d) => ({ id: d.id, ...d.data() } as Employee));
        const empIds = new Set(empList.map((e) => e.id));
        // Only show attendance records for workforce employees (from employees collection)
        const allRecords = attSnap.docs
          .map((d) => ({ id: d.id, ...d.data() } as AttRecord))
          .filter((r) => empIds.has(r.workerId));
  ...
  const filteredRecords = records.filter((r) => {
    ...
    let inRange = true;
    if (filter === "week") inRange = rDate >= weekStart(today) && rDate <= today;
    else if (filter === "month") inRange = rDate >= monthStart(today) && rDate <= today;
    else if (filter === "custom" && customFrom && customTo) inRange = r.date >= customFrom && r.date <= customTo;
    const empMatch = selectedEmployee === "all" || r.workerId === selectedEmployee;
  ...
  const earnedForRecord = (r: AttRecord) => {
    const emp = empMap[r.workerId];
    if (!emp || emp.salaryType !== "daily") return 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      {/* Header */}
      <div style={{ backgroundColor: GREEN }} className="px-4 pt-12 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-green-200 text-xs">{filteredRecords.length} records · {employees.length} employees</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 flex flex-col gap-3">
        ...
        <div className="flex gap-2">
          {([
            ...
            <button key={key} onClick={() => setFilter(key)}
              ...
              style={{
                backgroundColor: filter === key ? GREEN : "white",
        ...
        <div className="bg-white rounded-2xl px-4 py-2 shadow-sm">
          <select value={selectedEmployee} onChange={(e) => setSelectedEmployee(e.target.value)}
            ...
            {employees.map((emp) => (
              <option key={emp.id} value={emp.id}>{emp.name} — {typeLabel(emp)}</option>
            ))}
          </select>
        </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="grid grid-cols-4 gap-2">
              <div className="rounded-xl p-2 text-center" style={{ backgroundColor: "#E8F5E9" }}>
                <p className="text-xs font-medium" style={{ color: GREEN }}>Present</p>
                <p className="font-bold text-lg" style={{ color: GREEN }}>{totalPresent}</p>
              </div>
        ...
        {loading ? (
          ...
          <div className="text-center pt-10">
            ...
            <p className="text-gray-400 text-sm mt-1">Try a different date range or employee filter</p>
          </div>
        ) : (
